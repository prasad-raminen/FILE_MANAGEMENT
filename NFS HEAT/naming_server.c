            ...
            case MSG_EXEC_FILE: {
                ...
                if (connect(ss_sock, (struct sockaddr*)&ss_addr, sizeof(ss_addr)) == 0) {
                    ...
                    if (ss_resp.type == MSG_RESPONSE) {
                        // Parse file content - extract commands from words
                        // File contains sentences with words, we need to extract shell commands
                        char file_content[MAX_BUFFER * 4];
                        strcpy(file_content, ss_resp.data);
                        
                        response.data[0] = '\0';
                        
                        // Replace delimiters (;) with newlines for command separation
                        char content_copy[MAX_BUFFER * 4];
                        ...
                        char *command = strtok(content_copy, "\n");
                        while (command) {
                            // Trim leading/trailing spaces
                            int start = 0;
                            int end = strlen(command) - 1;
                            while (start <= end && command[start] == ' ') start++;
                            while (end >= start && command[end] == ' ') end--;
                            
                            if (start <= end) {
                                char trimmed_cmd[512];
                                strncpy(trimmed_cmd, &command[start], end - start + 1);
                                trimmed_cmd[end - start + 1] = '\0';
                                
                                // Skip empty commands and shell keywords
                                if (strlen(trimmed_cmd) > 0 && strcmp(trimmed_cmd, "ETIRW") != 0) {
                                    FILE *fp = popen(trimmed_cmd, "r");
                                    if (fp) {
                                        char line[512];
                                        while (fgets(line, sizeof(line), fp)) {
                                            if (strlen(response.data) + strlen(line) < sizeof(response.data) - 1) {
