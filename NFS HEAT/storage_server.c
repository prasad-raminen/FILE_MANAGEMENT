...
SentenceLock sentence_write_locks[MAX_FILES * 100];  // filename + sentence_num
int sentence_lock_count = 0;
pthread_mutex_t sentence_lock_list_mutex = PTHREAD_MUTEX_INITIALIZER;

int lock_sentence_for_write(const char *filename, int sentence_num) {
    pthread_mutex_lock(&sentence_lock_list_mutex);
    
    int idx = -1;
    for (int i = 0; i < sentence_lock_count; i++) {
        if (strcmp(sentence_write_locks[i].filename, filename) == 0 && 
            sentence_write_locks[i].sentence_num == sentence_num) {
            idx = i;
            break;
        }
    }
    
    if (idx == -1 && sentence_lock_count < MAX_FILES * 100) {
        idx = sentence_lock_count++;
        strcpy(sentence_write_locks[idx].filename, filename);
        sentence_write_locks[idx].sentence_num = sentence_num;
        sentence_write_locks[idx].locked = 0;
        pthread_mutex_init(&sentence_write_locks[idx].mutex, NULL);
    }
    
    if (idx == -1) {
        pthread_mutex_unlock(&sentence_lock_list_mutex);
        return -1;
    }
    
    pthread_mutex_lock(&sentence_write_locks[idx].mutex);
    pthread_mutex_unlock(&sentence_lock_list_mutex);
    
    if (sentence_write_locks[idx].locked) {
        pthread_mutex_unlock(&sentence_write_locks[idx].mutex);
        return -1;
    }
    
    sentence_write_locks[idx].locked = 1;
    pthread_mutex_unlock(&sentence_write_locks[idx].mutex);
    
    return idx;
}

void unlock_sentence_for_write(int lock_idx) {
    if (lock_idx >= 0 && lock_idx < sentence_lock_count) {
        pthread_mutex_lock(&sentence_write_locks[lock_idx].mutex);
        sentence_write_locks[lock_idx].locked = 0;
        pthread_mutex_unlock(&sentence_write_locks[lock_idx].mutex);
    }
}

int is_sentence_locked_for_write(const char *filename, int sentence_num) {
    pthread_mutex_lock(&sentence_lock_list_mutex);
    ...
    for (int i = 0; i < sentence_lock_count; i++) {
        if (strcmp(sentence_write_locks[i].filename, filename) == 0 && 
            ...
            pthread_mutex_lock(&sentence_write_locks[i].mutex);
            int locked = sentence_write_locks[i].locked;
            pthread_mutex_unlock(&sentence_write_locks[i].mutex);
    ...
    switch (msg.type) {
        ...
        case MSG_WRITE_FILE: {
            ...
            if (strlen(msg.data) == 0) {
                // Phase 1: Try to acquire sentence lock
                int sentence_lock_idx = lock_sentence_for_write(msg.filename, msg.sentence_num);
                if (sentence_lock_idx < 0) {
                    response.type = MSG_ERROR;
                    response.error_code = ERR_SENTENCE_LOCKED;
                    strcpy(response.data, "Sentence is currently being accessed by another user");
                    send_message(sockfd, &response);
                    close(sockfd);
                    return NULL;
                }
                ...
                if (receive_message(sockfd, &write_msg) < 0) {
                    unlock_sentence_for_write(sentence_lock_idx);
                    close(sockfd);
                    return NULL;
                }
            ...
            // Find the lock index for this sentence
            int sentence_lock_idx = -1;
            pthread_mutex_lock(&sentence_lock_list_mutex);
            for (int i = 0; i < sentence_lock_count; i++) {
                if (strcmp(sentence_write_locks[i].filename, msg.filename) == 0 &&
                    sentence_write_locks[i].sentence_num == msg.sentence_num) {
                    sentence_lock_idx = i;
                    break;
                }
            }
            ...
            if (msg.sentence_num < 0 || msg.sentence_num > num_sentences) {
                ...
                if (sentence_lock_idx >= 0) unlock_sentence_for_write(sentence_lock_idx);
                send_message(sockfd, &response);
                close(sockfd);
                return NULL;
            }
            ...
            while (line != NULL) {
                ...
                if (sscanf(line, "%d %[^\n]", &line_word_idx, word_content) == 2) {
                    // Parse each word in the input, handling delimiters to create multiple sentences
                    int content_idx = 0;
                    
                    // Skip leading spaces
                    while (word_content[content_idx] && word_content[content_idx] == ' ') {
                        content_idx++;
                    }
                    
                    // For this line, the first word uses line_word_idx
                    int current_word_idx = line_word_idx;
                    int first_word_in_line = 1;
                    
                    // Process all words from this line
                    while (word_content[content_idx]) {
                        char current_word[MAX_FILENAME] = "";
                        int word_len = 0;
                        int has_delimiter = 0;
                        
                        // Extract one word (including any delimiter attached to it)
                        while (word_content[content_idx] && word_content[content_idx] != ' ') {
                            current_word[word_len++] = word_content[content_idx];
                            // Check if this character is a sentence delimiter
                            if (word_content[content_idx] == '.' || 
                                word_content[content_idx] == '!' || 
                                word_content[content_idx] == '?') {
                                has_delimiter = 1;
                            }
                            content_idx++;
                        ...
                        if (word_len > 0) {
                            // For the first word in a line, allow flexible indexing
                            // If user specifies index 2 in empty sentence, just append at end
                            int array_idx;
                            if (first_word_in_line && current_word_idx > num_words + 1) {
                                // Index is beyond capacity - insert at end
                                array_idx = num_words;
                                first_word_in_line = 0;
                                current_word_idx = num_words + 2; // Next word will be at end
                            } else {
                                // Validate current_word_idx ONLY for current sentence
                                // Valid range: 1 to (num_words + 1) for insertion
                                if (current_word_idx < 1 || current_word_idx > num_words + 1) {
                                    response.type = MSG_ERROR;
                                    response.error_code = ERR_INVALID_INDEX;
                                    snprintf(response.data, sizeof(response.data), 
                                             "Word index %d out of range (valid: 1 to %d)", current_word_idx, num_words + 1);
                                    if (sentence_lock_idx >= 0) unlock_sentence_for_write(sentence_lock_idx);
                                    send_message(sockfd, &response);
                                    close(sockfd);
                                    return NULL;
                                ...
                                // Convert current_word_idx from 1-based to 0-based for array indexing
                                array_idx = current_word_idx - 1;
                                first_word_in_line = 0;
                            }
                            
                            // Insert word at the specified position (shift existing words)
                            ...
                            // Increment for next word in this line (sequential insertion after each word)
                            current_word_idx++;
                            
                            // If this word ends with a delimiter, finalize sentence and start new one
                            if (has_delimiter) {
                                // Reconstruct and save current sentence
                                ...
                                num_words = 0;
                                current_word_idx = 1;  // Next word in new sentence starts at index 1
                                first_word_in_line = 1; // Reset for new sentence
                            }
                        }
